        ...
        <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {collections.map((c, i) => (
            <Reveal key={c.name} delay={i * 100}>
              <TiltCard className="group rounded-2xl border border-noir/10 bg-card p-6 hover:border-orange hover:shadow-xl">
                <div className="mb-4 aspect-square rounded-xl bg-rose transition-transform duration-300 group-hover:scale-[1.03]" />
                <p className="font-display text-xl">{c.name}</p>
                <p className="mt-1 text-sm text-noir/60">{c.desc}</p>
              </TiltCard>
            </Reveal>
          ))}
        </div>
