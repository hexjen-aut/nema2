...
const stages = [
  { label: "Produit vierge", note: "Le modèle avant personnalisation" },
  { label: "Personnalisation", note: "Couleurs et détails composés" },
  { label: "Création finale", note: "Votre pièce, prête à être fabriquée" },
];

export default function Creez() {
  return (
          ...
          {stages.map((stage, i) => (
            <div key={stage.label} className="contents">
              ...
              {i < stages.length - 1 && (
                <span
                  aria-hidden="true"
                  className="hidden text-2xl text-orange md:block"
                >
                  →
                </span>
              )}
